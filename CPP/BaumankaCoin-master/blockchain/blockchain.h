...
namespace ad_patres
{
  ...
  public:
    ...
    void customize(size_t numberOfBlocks, Botan::secure_vector<uint8_t> address);

    static std::shared_ptr<Blockchain> instance();

    bool validateTxn(const Transaction&, std::vector<std::pair<Output, size_t>>);

    Botan::secure_vector<uint8_t> getLastBlockHash();
    ...
    std::vector<uint8_t> getLastBlockData() const;
  ...
  private:
    ...
    size_t validateInputs(const Transaction&, std::vector<std::pair<Output, size_t>>);
